...
import { validatePhysicsEditOperation } from '../src/physics-authoring.js';
...
describe('semantic physics authoring validation', () => {
  it('accepts the pinned SimplePhysics create contract without native identities', () => {
    expect(() => validatePhysicsEditOperation({
      type: 'physics.create',
      parentPath: '/Root/Head',
      name: 'Hair Physics',
      parameterName: 'HairSwing',
      ...settings,
    })).not.toThrow();
  });
  ...
  it('rejects unsupported models/map modes and non-physical numeric settings', () => {
    expect(() => validatePhysicsEditOperation({
      type: 'physics.create', parentPath: '/Root', name: 'P', parameterName: 'Swing',
      ...settings, model: 'verlet' as never,
    })).toThrow(InvalidBindingError);

    expect(() => validatePhysicsEditOperation({
      type: 'physics.create', parentPath: '/Root', name: 'P', parameterName: 'Swing',
      ...settings, mapMode: 'polar' as never,
    })).toThrow(InvalidBindingError);

    for (const bad of [
      { length: 0 }, { frequency: 0 }, { angleDamping: -0.1 }, { lengthDamping: -0.1 },
      { gravity: Number.NaN }, { outputScale: [1, Number.POSITIVE_INFINITY] as [number, number] },
    ]) {
      expect(() => validatePhysicsEditOperation({
        type: 'physics.update', path: '/Root/P', settings: bad,
      })).toThrow(InvalidBindingError);
    }
  });
});
