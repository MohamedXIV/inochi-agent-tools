...
private bool applyPreviewParameters(Puppet puppet, const(char)* valuesJson, ref JSONValue applied, out string error) {
    applied = JSONValue.emptyObject;
...
private JSONValue frameMetadata(Puppet puppet) {
    ...
    ulong drawableCommands, texturedCommands, normalCommands, maskDefinitionCommands, maskedCommands, compositeCommands;
    ...
    if (hasBounds) {
        minX = maxX = vertices[0].vtx.x; minY = maxY = vertices[0].vtx.y;
        foreach (ref vertex; vertices[1 .. $]) {
            minX=min(minX,vertex.vtx.x); minY=min(minY,vertex.vtx.y); maxX=max(maxX,vertex.vtx.x); maxY=max(maxY,vertex.vtx.y);
        }
        hasBounds=isFinite(minX)&&isFinite(minY)&&isFinite(maxX)&&isFinite(maxY);
    }
    JSONValue result=JSONValue.emptyObject;
    result["schemaVersion"]=1; result["kind"]="inochi2d-draw-list-frame";
    result["commandCount"]=cast(ulong)commands.length; result["drawableCommandCount"]=drawableCommands; result["texturedCommandCount"]=texturedCommands;
    result["vertexCount"]=cast(ulong)vertices.length; result["indexCount"]=cast(ulong)indices.length;
    result["states"]=JSONValue.emptyObject; result["states"]["normal"]=normalCommands; result["states"]["defineMask"]=maskDefinitionCommands; result["states"]["maskedDraw"]=maskedCommands; result["states"]["composite"]=compositeCommands;
    result["hasRenderableContent"]=drawableCommands>0&&vertices.length>=3&&indices.length>=3;
    if (hasBounds) { result["bounds"]=JSONValue.emptyObject; result["bounds"]["minX"]=minX; result["bounds"]["minY"]=minY; result["bounds"]["maxX"]=maxX; result["bounds"]["maxY"]=maxY; }
...
export extern(C) int iat_capture_preview_frame_json(const(char)* inputPath, char** outJson, char** outError) nothrow {
    if (outJson is null || outError is null) return 2; *outJson=null; *outError=null;
    if (inputPath is null) return fail(outError,2,"input path is required");
    try { auto puppet=inLoadPuppet!Puppet(fromStringz(inputPath).idup); if (puppet is null) return fail(outError,3,"failed loading puppet for preview capture"); scope(exit) destroy(puppet); puppet.update(0); puppet.draw(0); auto metadata=frameMetadata(puppet); *outJson=copyCString(toJSON(metadata)); return *outJson is null?fail(outError,1,"failed allocating preview frame JSON"):0; }
    catch(Throwable error) return fail(outError,1,"preview frame capture failed: "~error.msg);
}

private float edge(float ax,float ay,float bx,float by,float px,float py) { return (px-ax)*(by-ay)-(py-ay)*(bx-ax); }

export extern(C) int iat_render_preview_png_json(const(char)* inputPath,const(char)* outputPath,uint width,uint height,const(char)* valuesJson,char** outJson,char** outError) nothrow {
    if(outJson is null||outError is null)return 2; *outJson=null; *outError=null;
    ...
    try {
        auto puppet=inLoadPuppet!Puppet(fromStringz(inputPath).idup); if(puppet is null)return fail(outError,3,"failed loading puppet for preview render"); scope(exit)destroy(puppet);
        puppet.update(0);
        JSONValue appliedParameters; string parameterError;
        if(!applyPreviewParameters(puppet,valuesJson,appliedParameters,parameterError))return fail(outError,10,parameterError);
        puppet.update(0); puppet.draw(0);
        auto dl=puppet.drawList; auto meta=frameMetadata(puppet); meta["appliedParameters"]=appliedParameters;
        if(!meta["hasRenderableContent"].boolean)return fail(outError,4,"preview frame has no renderable content");
        auto bounds=meta["bounds"]; float minX=cast(float)bounds["minX"].floating,minY=cast(float)bounds["minY"].floating,maxX=cast(float)bounds["maxX"].floating,maxY=cast(float)bounds["maxY"].floating;
        float spanX=max(maxX-minX,0.001f),spanY=max(maxY-minY,0.001f),scale=min((width-16.0f)/spanX,(height-16.0f)/spanY),ox=(width-spanX*scale)*0.5f-minX*scale,oy=(height-spanY*scale)*0.5f+maxY*scale;
        ...
        if(coveredPixels==0)return fail(outError,4,"preview rasterized to an empty image"); write_image(fromStringz(outputPath).idup,width,height,pixels,4);
        meta["kind"]="inochi2d-headless-preview"; meta["width"]=width; meta["height"]=height; meta["triangleCount"]=triangles; meta["coveredPixelSamples"]=coveredPixels; meta["output"]=fromStringz(outputPath).idup; *outJson=copyCString(toJSON(meta)); return *outJson is null?fail(outError,1,"failed allocating preview render JSON"):0;
