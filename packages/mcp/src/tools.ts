...
function tool(name: string, description: string, inputSchema: Record<string, unknown>, handler: (input: UnknownRecord) => Promise<unknown>, validate: (input: UnknownRecord) => string | null): McpSemanticTool {
  return { name, description, inputSchema, async call(arguments_: unknown): Promise<McpToolResult> {
    ...
    try { return { ok: true, result: await handler(input) }; } catch (error) { return semanticFailure(error); }
  } };
}

export function createMcpToolRegistry(client: AuthoringClient): McpSemanticTool[] {
  const inputOnlySchema = objectSchema({ inputPath: stringFieldSchema('Path to a real Inochi .inp puppet') }, ['inputPath']);
  ...
  return [
    ...
    tool('puppet.validate', 'Validate and inspect a real Inochi puppet.', inputOnlySchema, (input) => client.validatePuppet({ inputPath: input.inputPath as string }), (input) => requiredString(input, 'inputPath') ? null : 'puppet.validate requires inputPath'),
    tool('puppet.save', 'Save a real Inochi puppet to a distinct .inp path.', objectSchema({ inputPath: stringFieldSchema('Source .inp path'), outputPath: stringFieldSchema('Destination .inp path') }, ['inputPath', 'outputPath']), (input) => client.savePuppet({ inputPath: input.inputPath as string, outputPath: input.outputPath as string }), (input) => {
    ...
    tool('puppet.edit', 'Apply semantic authoring operations and save to a distinct real .inp path.', objectSchema({ inputPath: stringFieldSchema('Source .inp path'), outputPath: stringFieldSchema('Destination .inp path'), operations: { type: 'array', minItems: 1, items: { type: 'object' } } }, ['inputPath', 'outputPath', 'operations']), (input) => client.editPuppet({ inputPath: input.inputPath as string, outputPath: input.outputPath as string, operations: input.operations as Parameters<AuthoringClient['editPuppet']>[0]['operations'] }), (input) => {
    ...
    tool('preview.render', 'Render a deterministic headless PNG preview at an optional semantic parameter state.', objectSchema({ inputPath: stringFieldSchema('Source .inp path'), outputPath: stringFieldSchema('Destination PNG path'), width: { type: 'integer', minimum: 16, maximum: 4096 }, height: { type: 'integer', minimum: 16, maximum: 4096 }, parameters: { type: 'object', additionalProperties: { type: 'array', minItems: 2, maxItems: 2, items: { type: 'number' } } } }, ['inputPath', 'outputPath']), (input) => client.renderPreview({ inputPath: input.inputPath as string, outputPath: input.outputPath as string, ...(input.width === undefined ? {} : { width: input.width as number }), ...(input.height === undefined ? {} : { height: input.height as number }), ...(input.parameters === undefined ? {} : { parameters: input.parameters as Parameters<AuthoringClient['renderPreview']>[0]['parameters'] }) }), (input) => {
      ...
      for (const key of ['width', 'height'] as const) { const value = input[key]; if (value !== undefined && (!Number.isInteger(value) || (value as number) < 16 || (value as number) > 4096)) return `preview.render ${key} must be an integer between 16 and 4096`; }
      if (input.parameters !== undefined && !record(input.parameters)) return 'preview.render parameters must be an object';
      return null;
    }),
    tool('parameter.evaluate', 'Apply parameter values, report bound target values, and verify restoration.', objectSchema({ inputPath: stringFieldSchema('Source .inp path'), values: { type: 'object', minProperties: 1, additionalProperties: { type: 'array' } } }, ['inputPath', 'values']), (input) => client.evaluateParameters({ inputPath: input.inputPath as string, values: input.values as Parameters<AuthoringClient['evaluateParameters']>[0]['values'] }), (input) => {
