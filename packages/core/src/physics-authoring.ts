...
export type PhysicsEditOperation =
  | PhysicsCreateOperation
  | PhysicsUpdateOperation
  | PhysicsRemoveOperation;
...
function validateSettings(settings: Partial<SimplePhysicsSettings>, requireAll: boolean): void {
  ...
  if (settings.model !== undefined && !MODELS.has(settings.model)) {
    throw new InvalidBindingError(`Unsupported physics model: ${String(settings.model)}`);
  }
  if (requireAll && settings.mapMode === undefined) throw new InvalidBindingError('Physics map mode is required');
  if (settings.mapMode !== undefined && !MAP_MODES.has(settings.mapMode)) {
    throw new InvalidBindingError(`Unsupported physics map mode: ${String(settings.mapMode)}`);
  }
  ...
  for (const [key, label] of [
    ...
    ['angleDamping', 'Physics angle damping'],
    ['lengthDamping', 'Physics length damping'],
  ] as const) {
  ...
  if (settings.length !== undefined && settings.length <= 0) {
    throw new InvalidBindingError('Physics length must be greater than zero');
  }
  if (settings.frequency !== undefined && settings.frequency <= 0) {
    throw new InvalidBindingError('Physics frequency must be greater than zero');
  }
  if (settings.angleDamping !== undefined && settings.angleDamping < 0) {
    throw new InvalidBindingError('Physics angle damping must not be negative');
  }
  if (settings.lengthDamping !== undefined && settings.lengthDamping < 0) {
    throw new InvalidBindingError('Physics length damping must not be negative');
  }

  if (requireAll && settings.outputScale === undefined) {
    throw new InvalidBindingError('Physics output scale is required');
  }
  if (settings.outputScale !== undefined &&
      (!Array.isArray(settings.outputScale) || settings.outputScale.length !== 2 ||
       !settings.outputScale.every(Number.isFinite))) {
    throw new InvalidBindingError('Physics output scale must be a finite numeric pair');
  }
  if (requireAll && settings.localOnly === undefined) {
    throw new InvalidBindingError('Physics local-only flag is required');
  }
}

/** Validate the public semantic contract before any native mutation occurs. */
export function validatePhysicsEditOperation(operation: PhysicsEditOperation): void {
  switch (operation.type) {
    ...
    case 'physics.update':
      ...
      if (Object.keys(operation.settings).length === 0 && operation.parameterName === undefined) {
        throw new InvalidAuthoringRequestError('Physics update must change a setting or target parameter');
      }
