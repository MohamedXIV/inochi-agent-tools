...
extern(C) int iat_render_preview_png_json(const(char)* inputPath, const(char)* outputPath, uint width, uint height, char** outJson, char** outError);
...
private int emitResult(int result, char* json, char* error, string fallback) {
    if (result != 0) {
        if (error !is null) { stderr.writeln(fromStringz(error)); iat_string_free(error); }
        else stderr.writeln(fallback);
        if (json !is null) iat_string_free(json);
        return result;
    }
    if (json is null) {
        if (error !is null) iat_string_free(error);
        stderr.writeln(fallback ~ " returned no JSON"); return 1;
    }
    stdout.write(fromStringz(json)); iat_string_free(json);
    if (error !is null) iat_string_free(error);
    return 0;
}
...
private int runInspect(string path) {
    ...
    if (result != 0) {
        if (error !is null) { stderr.writeln(fromStringz(error)); iat_string_free(error); } else stderr.writeln("native puppet inspection failed");
        if (json !is null) iat_string_free(json); return 3;
    }
...
private int runRenderPreview(string inputPath,string outputPath,string width,string height) {
    char* j; char* e;
    try return emitResult(iat_render_preview_png_json(toStringz(inputPath),toStringz(outputPath),width.to!uint,height.to!uint,&j,&e),j,e,"native preview render failed");
    catch (Exception ex) { stderr.writeln("invalid preview dimensions: ",ex.msg); return 2; }
}
private int runSaveAs(string inputPath,string outputPath) {
    char* error; auto result=iat_save_puppet(toStringz(inputPath),toStringz(outputPath),&error);
    if (result != 0) { if (error !is null) { stderr.writeln(fromStringz(error)); iat_string_free(error); } else stderr.writeln("native puppet save-as failed"); return result; }
    if (error !is null) iat_string_free(error); return runInspect(outputPath);
}
...
int main(string[] args) {
    ...
    stderr.writeln("usage: iat_native_host inspect <puppet-path> | create-minimal <output.inp> <name> | save-as <input.inp> <output.inp> | edit-visual <input.inp> <output.inp> <operations-json> | evaluate-parameters <input.inp> <values-json> | capture-preview-frame <input.inp> | render-preview <input.inp> <output.png> <width> <height>");
    return 2;
}
